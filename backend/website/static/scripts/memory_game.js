...
console.log("memory_game.js chargé");

// Fonction pour charger et exécuter le script externe
async function loadAndExecuteScript(url) {
    const response = await fetch(url);
    let scriptContent = await response.text();

    // Ajouter le script au DOM pour l'exécuter
    ...
    // Supprimer le script du DOM après son exécution
    document.body.removeChild(scriptTag);
}

// URL du script externe à charger
const animeJsUrl = 'https://cdnjs.cloudflare.com/ajax/libs/animejs/2.0.2/anime.min.js';

// Charger et exécuter le script externe
loadAndExecuteScript(animeJsUrl).then(() => {
    console.log("AnimeJS chargé et prêt à être utilisé.");
...
displayPlayer();  
    ...
    console.error("Une erreur s'est produite lors du chargement du script externe:", error);
}   );