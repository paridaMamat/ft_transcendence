...
loadScript("https://cdnjs.cloudflare.com/ajax/libs/three.js/r128/three.min.js")
	...
	.then(() => {
		...
		let ballSpeed = 1; // Vitesse de la balle
		let scorePlayer1 = 0;
		let scorePlayer2 = 0;
		//updateScore();
		...
		socket.onmessage = function(e) {
			...
			if (data.type === 'game_state') {
				...
				// Update the game based on the received state
				console.log('Received game state:', state);
				updatePaddles(state.paddles);
				updateBall(state.ball_position);
		...
		document.addEventListener('keydown', function(event) {
            if (event.key === 'ArrowUp') {
                raquette2.position.z = Math.min(raquette2.position.z + 10, 125 / 2);
                sendMessage('move_paddle', {'side': 'right', 'position': raquette2.position.z});
            } else if (event.key === 'ArrowDown') {
                raquette2.position.z = Math.max(raquette2.position.z - 10, -125 / 2);
                sendMessage('move_paddle', {'side': 'right', 'position': raquette2.position.z});
            }
			if (event.key === 'w') {
				raquette1.position.z = Math.min(raquette1.position.z + 10, 125 / 2);
				sendMessage('move_paddle', { 'side': 'left', 'position': raquette1.position.z });
			}
			else if (event.key === 's') {
				raquette1.position.z = Math.max(raquette1.position.z - 10, -125 / 2);
				sendMessage('move_paddle', { 'side': 'left', 'position': raquette1.position.z });
			}
		});
		// functions to handle game state updates
		function updatePaddles(paddles) {
			// Update the positions of the paddles
			console.log("left paddle moved", paddles.left.y);
			console.log("right paddle moved", paddles.right.y);
			raquette1.position.z = paddles.left.y;
            raquette2.position.z = paddles.right.y;
		}

		function updateBall(position) {
			// Update the position of the ball
			console.log("ball moved in updateBall", position);
			balle.position.set(position.x, balle.position.y, position.z);
		}

		function scoreUpdate(score) {
			// // Update the score display
			console.log("Updating score:", score);
			scorePlayer1 = score.left;
            scorePlayer2 = score.right;
		...
		// 		scorePlayer2 = 0;
		// 	}
		// 	console.log("resetGame deuxieme");
		// }
